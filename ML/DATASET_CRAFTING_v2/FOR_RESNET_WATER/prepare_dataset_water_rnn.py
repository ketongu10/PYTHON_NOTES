...
TOTAL_LIMITS = 0
TOTAL_PREVIOUS_FALSE = TOTAL_PREVIOUS_TRUE = 0
...
def hysteresis(link, data, smoke):
    frame_delta = int(data[1].split()[0]) - int(data[0].split()[0])
    dT = 25     # frames = 1 sec
    index_shift = np.ceil(dT / frame_delta).astype(int)
    sum = 0
    # IF AVERAGE data a second ago FOR 2 seconds was 0.5, then THERE WAS A WATER
    for ind in range(max(link-index_shift, -1), max(link-3*index_shift, -1), -1):
        is_water_old = int(data[ind].split()[1])> 0
        check_smoke = smoke and is_water_old and int(data[ind].split()[0]) in list(smoke.keys())  # there is smoke that doesn't cover water
        sum += 1 if (is_water_old and not smoke or check_smoke) else 0
        if sum >= index_shift:
            return "previous_"
    return ""
...
def check_mask(link, data, net_buf, smoke):
    ...
    for i in range(net_buf):
        ...
        bo = bo and (sample[i] > 0 and not smoke or check_smoke)
    if bo:
        return 1
...
def gen_targets_from_vidos(label_path, label_dir, available_name):
    ...
    link = 0
    name_shift = 0
    global TOTAL_PREVIOUS_FALSE
    global TOTAL_PREVIOUS_TRUE
    while link + net_buf <= len(old_data):
        result = check_mask(link, old_data, net_buf, smoke_lbs)
        if result == 0: #MEANS IT IS TRESH - TRY AGAIN
            link+=1
            continue
        if (result == 1 or result == -1):
            if np.random.uniform() < DATASET_GEN_P:
                hysteresis_marker = hysteresis(link, old_data, smoke_lbs)
                if hysteresis_marker:
                    if result == -1:
                        TOTAL_PREVIOUS_FALSE+=1
                    else:
                        TOTAL_PREVIOUS_TRUE+=1
                cur_name = hysteresis_marker + down_marker + str(available_name+name_shift)
                create_target(label_dir, old_data, link, net_buf, result, cur_name, label_v2, smoke_lbs)
                name_shift+=1
                link+=net_buf
                continue
            else:   #IF WE WANT TO OMIT SOME DATA TO PREVENT SIMILAR-LOOKING CASES IN DATASET
                link += net_buf
                continue
    return name_shift
...
"From PC smoke/water_flow",
"From 106 smoke/water_flow",]
dirs = ["From 106 24.01 smoke/water_flow", "From 106 smoke/water_flow", "From PC smoke/water_flow"]
print(len(dirs))
print(dirs)
os.mkdir(dataset)
...
print(f"TOTAL PREVIOUS TRUE: {TOTAL_PREVIOUS_TRUE}")
print(f"TOTAL PREVIOUS FALSE: {TOTAL_PREVIOUS_FALSE}")