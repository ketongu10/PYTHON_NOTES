...
path = "/vol1/KSH/dataset/106 PROC 10.02"

#paths_of_images = [image for image in os.listdir(path+"/images/val")]
sum = 0


sh_progress = SharedMemory("shared_progress_crop", create=True, size=8)
c = np.ndarray((1,), dtype=np.int64, buffer=sh_progress.buf)
c[0] = 0
def doit(image_path):
    sh_progress_pool = SharedMemory("shared_progress_crop", create=False)
