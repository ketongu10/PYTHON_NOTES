...
def unite_bboxes(bbox1, bbox2):
    ...
    x3, y3, w3, h3, s3, inds3 = (b3_x1 + b3_x2) / 2, (b3_y1 + b3_y2) / 2, (b3_x2 - b3_x1), (b3_y2 - b3_y1), s1 + s2, (
        np.hstack((inds1[0], inds2[0])), np.hstack((inds1[1], inds2[1])))
...
bldr_clss_as_yolo_clss = {
    "pumka_base": {'classes': ["pumka_base"], 'task': 'FOR_EACH'},
    "pumka_square": {'classes': ["pumka_square"], 'task': 'FOR_EACH'},
    "pumka_sphere": {'classes': ["pumka"], 'task': 'FOR_EACH'},
    "pumka_long": {'classes': ["pumka_long"], 'task': 'FOR_EACH'},
    ...
    "pipe_head": {'classes': ["ksh_head_last",
                               "not_ksh_knot_last"], 'task': 'FOR_EACH'},
...
coef = 0.1
min_volume = {"pumka_base": 2269 * coef,
              "pumka_square": 1583 * coef,
              "pumka_sphere": 2657 * coef,
              "pumka_long": 2704 * coef,  # 11497*coef,
              "spider": 10000 * coef,  # 11497*coef,
              "pipe_1_end": 5000 * coef,  # 11497*coef,
              "pipe_head": 1583 * coef,  # 11497*coef,
              }
        ...
        for clss in bldr_clss_as_yolo_clss[key]['classes']:
            for psind in clss2psinds[clss]:
                ...
                if (len(pixel_inds[0]) * len(pixel_inds[1]) != 0):
                    ...
                    # else:
                    bboxes.append([xc, yc, w, h, len(pixel_inds[0]), pixel_inds])
        # if key != "gis_tros" and key != "ecn_tros":
...
def doit(dir):
    ...
    if os.path.exists(rooot + "/images/" + dir) and os.path.exists(rooot + "/masks/" + dir) and os.path.exists(
            rooot + "/jsons/" + dir):
        img_name = os.listdir(rooot + "/images/" + dir)[0]
        shutil.copy(os.path.join(rooot, "images/", dir, img_name),
                    os.path.join(dataset, "images/train/", dir + f"_{dataset_name}.jpg"))
        ...
        for key in yolo_class_indexes.keys():
            ...
            if (label[key] is not None):
                ...
                if (((key not in ("pumka_sphere", "pumka_long") or
                    key in ("pumka_sphere", "pumka_long") and
                    label.get("pumka_square") and
                    border_collision_is_passed(label["pumka_square"], "pumka_square")
                ) and ((key == "pipe_head" and label.get("pumka_square") is None) or key != "pipe_head")
                    ...
                    shutil.move(os.path.join(dataset, "images/train/", dir + f"_{dataset_name}.jpg"),
                                os.path.join(tresh,
                                             dir + f"_{dataset_name}_SMALL_OR_NOT_ALL_PARTS_OR_BOUND_INTERSECTION.jpg"))
                    return dataset + f"/{segment_labels_name}/train/" + dir + f"_{dataset_name}.txt"
...
for dataset_name in dataset_names:
    rooot = f"/vol2/PUMKA/BLENDER_DATASETS/source/06.25/{dataset_name}/tkrs_pumka"
    # rooot = f"/home/popovpe/blender-4.1.1-linux-x64/results/{dataset_name}/tkrs_pumka"
    ...
    data_to_process = os.listdir(os.path.join(rooot, "images"))[::]  # 53300
    ...
    print(f'TIMEOUT FOUND = {c[-2]-1}')
    for key in yolo_class_indexes.keys():
        c_ind = list(yolo_class_indexes.keys()).index(key)
        print(
            f'{key}: AVER PIX = {c[2 * c_ind] // c[2 * c_ind + 1]} | NOW PIXELS = {min_volume.get(key)} | TOTAL FOUND = {c[2 * c_ind + 1]}')
    with open(dataset + "/dataset_info.txt", "w+") as f:
        print(f'TOTAL IMAGES FOUND = {c[-1]-1} | MOVED TO TRESH = {treshed}', file=f)
        print(f'TIMEOUT FOUND = {c[-2]-1}', file=f)
        for key in yolo_class_indexes.keys():
            c_ind = list(yolo_class_indexes.keys()).index(key)
            print(
                f'{key}: AVER PIX = {c[2 * c_ind] // c[2 * c_ind + 1]} | NOW PIXELS = {min_volume.get(key)} | TOTAL FOUND = {c[2 * c_ind + 1]}',
