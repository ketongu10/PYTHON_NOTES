...
checkpoint_path = ("/home/popovpe/Projects/WaterTrain/owl.guard.cv/cyclops/training/liquid_seg/experiments/segformer_w_real/outputs/last.ckpt")
...
with (torch.no_grad()):
    ...
    for i in tqdm(range(len(val_dataset))):
        ...
        dump[:h, :] = (mask*255).numpy().astype(int)
        dump[h:, :] = (new_mask*255).astype(int)
        cv2.imwrite(str(test_path/f"{i}.png"), dump)

        gts = torch.tensor(np.array(mask.numpy()[None, ...]))
