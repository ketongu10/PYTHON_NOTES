...
CONVNEXT_PATH = Path(__file__).parent.parent/'base/convnext_from_prod.ckpt'
...
class SegmentationModel(pl.LightningModule):
    def __init__(self, validation_keys=None):
        ...
        self.model = CustomEncoderSegformer(CONVNEXT_PATH, num_classes=len(CLS_KEYS))
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), weight_decay=1e-4, lr=1e-5)
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 30, gamma=0.2) #ExponentialLR(optimizer, gamma=0.912)
