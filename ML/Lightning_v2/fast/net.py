...
MOBILENET_PATH = Path(__file__).parent/'mobilenet_v3l_pretrained.ckpt'
...
class SegmentationModel(pl.LightningModule):
    def __init__(self, is_ft=True):
        ...
        print(MOBILENET_PATH)
        self.model = CustomEncoderSegformer(MOBILENET_PATH, num_classes=len(CLS_KEYS))
        self.is_ft = is_ft
        self.criterion = (
            nn.BCELoss()
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        #     [1 << i for i in range(len(CLS_KEYS))],
        #     dtype=torch.uint8,
        #     device=masks.device
        # ).reshape(len(CLS_KEYS), 1, 1)
        # unpacked_masks = (masks.unsqueeze(1) & bitmask) != 0  # Форма (B, C, H, W)
        # masks = unpacked_masks.to(torch.float32)

        outputs = self(images)
        if self.is_ft:
            loss = self.criterion(outputs[:, 0], masks[:, 0])
        else:
    ...
    #     print('INDS: ',self.inds['loh'], current_process().pid)
    #     print('IND_LOGGER: ',self.ram_loader.indexes_logger['loh'], current_process().pid)
    ...
    def on_train_epoch_end(self):
        ...
        print('train_end:', self.ram_loader.indexes_logger[:5])
    ...
    def on_validation_epoch_end(self):
        ...
        print(
            f"{self.trainer.current_epoch}: test_d_loss={Tloss}, test_d_rec={recc}, test_d_prec={prec}, test_d_f1={f1}")
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), weight_decay=1e-6, lr=3e-4)
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 25,
                                                       gamma=0.2)  # ExponentialLR(optimizer, gamma=0.912)
